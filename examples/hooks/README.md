...
quiver -i "do something"  # See tool calls logged to stderr
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PostToolUse.sh .quiver/hooks/
chmod +x .quiver/hooks/PostToolUse.sh
quiver -i "do something"  # See tool results logged and enriched
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PreToolUse_BlockDestructive.sh .quiver/hooks/PreToolUse.sh
chmod +x .quiver/hooks/PreToolUse.sh
quiver -i "clean up the repo"  # Destructive operations will be blocked
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PreToolUse_RequireReview.sh .quiver/hooks/PreToolUse.sh
chmod +x .quiver/hooks/PreToolUse.sh
quiver -i "update dependencies"  # Critical file writes will pause for review
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PreToolUse_InjectFileContext.sh .quiver/hooks/PreToolUse.sh
chmod +x .quiver/hooks/PreToolUse.sh
quiver -i "review the configuration"  # Related files will be mentioned automatically
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/TaskStart.sh .quiver/hooks/
cp examples/hooks/TaskComplete.sh .quiver/hooks/
cp examples/hooks/SessionShutdown.sh .quiver/hooks/
chmod +x .quiver/hooks/Task*.sh .quiver/hooks/SessionShutdown.sh
quiver -i "do something"  # Session lifecycle will be logged
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PreToolUse.py .quiver/hooks/
chmod +x .quiver/hooks/PreToolUse.py
quiver -i "do something"  # Python hook will log tool calls
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PostToolUse.py .quiver/hooks/
chmod +x .quiver/hooks/PostToolUse.py
quiver -i "do something"  # Python hook will enrich tool results
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PreToolUse_InjectContext.py .quiver/hooks/PreToolUse.py
chmod +x .quiver/hooks/PreToolUse.py
quiver -i "add a new feature"  # Related files and environment will be injected
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PreToolUse.ts .quiver/hooks/
chmod +x .quiver/hooks/PreToolUse.ts
quiver -i "do something"  # TypeScript hook will execute via bun
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PostToolUse.ts .quiver/hooks/
chmod +x .quiver/hooks/PostToolUse.ts
quiver -i "do something"  # TypeScript hook will execute via bun
```
...
```bash
mkdir -p .quiver/hooks
cp examples/hooks/PreToolUse_ModifyInput.ts .quiver/hooks/PreToolUse.ts
chmod +x .quiver/hooks/PreToolUse.ts
quiver -i "install dependencies"  # npm install will have --save-exact added automatically
```
...
**File hooks** go in `.quiver/hooks/` and must be named after the event they handle:

```bash
mkdir -p .quiver/hooks

# Copy PreToolUse example (pick your language)
cp examples/hooks/PreToolUse.sh .quiver/hooks/PreToolUse.sh      # Bash
cp examples/hooks/PreToolUse.py .quiver/hooks/PreToolUse.py      # Python
cp examples/hooks/PreToolUse.ts .quiver/hooks/PreToolUse.ts      # TypeScript

# Copy PostToolUse example
cp examples/hooks/PostToolUse.sh .quiver/hooks/PostToolUse.sh    # Bash
cp examples/hooks/PostToolUse.py .quiver/hooks/PostToolUse.py    # Python
cp examples/hooks/PostToolUse.ts .quiver/hooks/PostToolUse.ts    # TypeScript
```

### 2. Make it executable

```bash
chmod +x .quiver/hooks/PreToolUse.*
chmod +x .quiver/hooks/PostToolUse.*
```

### 3. Test it

```bash
quiver -i "test prompt"
# Or load from a custom hooks directory:
quiver --hooks-dir ./my-hooks -i "test prompt"
```
...
```bash
quiver --verbose "your prompt"
```

### Test a hook manually
```bash
echo '{"tool_call": {"name": "read_files", "input": {"filePath": "test.ts"}}}' | .quiver/hooks/PreToolUse.sh
```

### Check hook output
```bash
.quiver/hooks/PreToolUse.sh < input.json | jq .
```
...
```bash
quiver plugin install https://github.com/quiverdev/quiver/blob/main/sdk/examples/hooks/custom-compaction-hook.example.ts --cwd .

quiver -i "Search the codebase for dispatcher usage, then summarize it"
```
...
| ------- | --------------- | ------------- | -------- |
| `custom-compaction-hook.example.ts` (in `.quiver/plugins/`) | `hooks.beforeModel` runtime hook | Agent runtime request messages with runtime parts such as `tool-call`, `tool-result`, `reasoning`, `image`, and `file` | Cases needing runtime-hook context, current runtime snapshot, or direct request mutation |
...
- **Use multiple hooks** — different event files can coexist in `.quiver/hooks/`
