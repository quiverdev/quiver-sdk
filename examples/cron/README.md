# Quiver Automation Examples

This directory contains example automation specs for file-based and event-driven automation in Quiver. Use these as templates to set up your own recurring or event-driven tasks.
...
Quiver automation supports two types of specs:

1. **Recurring specs** (`.cron.md`) — Run on a schedule
2. **Event-driven specs** (`.event.md`) — Run when an event occurs

Both can be enabled in `.quiver/cron/` to be picked up by the hub or SDK.
...
```bash
mkdir -p ~/.quiver/cron
cp examples/cron/daily-code-review.cron.md ~/.quiver/cron/
# Edit the spec: set workspaceRoot, model, etc.
# Spec is reconciled on startup; next run enqueued automatically
```

**One-off specs:**
For one-time tasks, save as `.quiver/cron/<name>.md` (no `.cron` infix) and omit the `schedule` field.
...
Event-driven specs live in `.quiver/cron/events/` and trigger when normalized events are ingested.
...
cp examples/cron/events/pr-review.event.md ~/.quiver/cron/events/
...
- Connector adapters
- `quiver.automation.ingestEvent()` in the SDK
...
```bash
mkdir -p ~/.quiver/cron/events
cp examples/cron/events/local-manual-test.event.md ~/.quiver/cron/events/

# Start the hub with automation enabled
# In another shell, ingest a test event
node -e "
  const { HubWebSocketClient } = require('@quiver/core');
...
```bash
mkdir -p ~/.quiver/cron/events
cp examples/cron/events/local-plugin-event.event.md ~/.quiver/cron/events/

# Load the plugin that emits these events
quiver plugin install https://github.com/quiverdev/quiver/blob/main/sdk/examples/plugins/automation-events.ts

# Run CLI with automation enabled; the plugin emits events
quiver --enable-automation -i "Test automation events"
```
...
**In the SDK:**
```ts
const quiver = await QuiverCore.create({
...
**In the CLI:**
```bash
quiver --enable-automation
```

### 5. Monitor runs

Completed and failed runs are reported to `.quiver/cron/reports/<run-id>.md` with:
...
- [Quiver SDK Examples](../) — Other integration examples
